...
function pdfFile(name = "test.pdf", content = "test pdf content"): File {
  const bytes = new TextEncoder().encode(`%PDF-1.4\n${content}`);
  return new File([bytes], name, { type: "application/pdf" });
}

function docxFile(name = "test.docx", content = "docx content"): File {
...
function jpgFile(name = "image.jpg"): File {
...
describe("POST /documents (BE-S2-01)", () => {
  ...
  test("AC-01.06: file exceeding max limit returns 422 FILE_TOO_LARGE", async () => {
    ...
    const file = new File([combined], "berkas-25mb.pdf", { type: "application/pdf" });
