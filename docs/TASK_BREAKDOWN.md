...
| Card ID | PM Card Title | Task Description | AC | Owner | Est | Docs |
|---|---|---|---|---|---|---|
| BE-S3-01 | Build the worker and the processing state machine | `apps/worker` consuming BullMQ. The four states and no others, with transitions guarded on the current state so a delayed duplicate job cannot move a READY document backwards. Three retries with exponential backoff on transient failures only, staying PROCESSING throughout so the user never sees a flicker. Idempotent `process`, every stage individually re-runnable. Reap rows left in QUEUED with no job by a crash between insert and the deferred enqueue (at most 20 per the batch cap). | AC-44.01, AC-44.02, AC-44.03 | BE1 | 3 | [technical-specs/12-document-processing-pipeline.md](technical-specs/12-document-processing-pipeline.md) 12.1 to 12.3; [api-specs/05-documents.md](api-specs/05-documents.md) 5.2 |
